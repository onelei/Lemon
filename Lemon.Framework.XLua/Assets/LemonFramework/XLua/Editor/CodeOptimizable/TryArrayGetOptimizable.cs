...
{
    public class TryArrayGetOptimizable : IOptimizable
    {
        public void Backup()
        {
        ...
        public void Optimization()
        {
            ...
            if (!File.Exists(filePath))
            {
                Debug.LogError("[CodeOptimizableHandler] WrapPusher.cs not exist");
                return;
            }
        ...
        private List<string> Generate(List<string> lines)
        {
            ...
            optLines.Insert(++index, "\t\t\tif (__ArrayGetDict.TryGetValue(type, out var action))");
            optLines.Insert(++index, "\t\t\t{");
            optLines.Insert(++index, "\t\t\t\taction?.Invoke(L,translator,obj,index);");
            optLines.Insert(++index, "\t\t\t\treturn true;\n\t\t\t}\n");

            index = flagIndex;
            optLines.Insert(index,
                "\n\t\tprivate static System.Collections.Generic.Dictionary<Type, System.Action<RealStatePtr,ObjectTranslator,object,int>> __ArrayGetDict = new System.Collections.Generic.Dictionary<Type, System.Action<RealStatePtr,ObjectTranslator,object,int>>{\n");
            while (typeActionEnumerator.MoveNext())
            {
                var key = typeActionEnumerator.Current.Key;
                optLines.Insert(++index, $"\t\t\t{{{key}, (L,translator,obj,index) => {{");
